...
function updateRootRelativeValues(state, sources, projectDirectoryRoot = state.projectDirectoryRoot, projectDirectoryRootName = state.projectDirectoryRootName) {
...
const queryAllDisplayedSources = (0, _resource.makeShallowQuery)({
  filter: (_, {
    sourcesWithUrls
  }) => sourcesWithUrls,
  map: (0, _resource.makeMapWithArgs)((resource, ident, {
    projectDirectoryRoot,
    ...
    id: resource.id,
    displayed: (0, _source.underRoot)(resource, projectDirectoryRoot, threads) && (!resource.isExtension || chromeAndExtensionsEnabled || debuggeeIsWebExtension)
  })),
  reduce: items => items.reduce((acc, {
    id,
    displayed
  }) => {
    if (displayed) {
...
function getAllDisplayedSources(state) {
  return queryAllDisplayedSources(state.sources.sources, {
    ...
    threads: (0, _threads.getAllThreads)(state)
  });
}
